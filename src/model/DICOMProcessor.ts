...
export class SingleDICOM {
  ...
  readDICOM = (input: InputType): dicomPerser.DataSet | void => {
  ...
  getImgInfoFromDICOM(dicom: dicomPerser.DataSet): ImageStructure {
  ...
  //databaseにdicomを格納する
  sendDICOMtoDB = (dicom: dicomPerser.DataSet): void => {
    testDB.dicom.push(dicom);
  }
  
  //databaseにimageStructureを格納する
  sendImgInfoToDB = (imageStructure: ImageStructure): void => {
